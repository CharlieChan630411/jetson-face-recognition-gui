...
class FaceDetector:
    def __init__(self):
        # 目前尚未導入 RetinaFace，我們預留 engine 載入的入口
        engine_path = "models/retinaface.engine"  # 預期的 engine 路徑（尚未存在）

        # 顯示目前僅為準備階段的 log
        print("🚧 注意：目前使用的是 face_recognition 模型")
        print(f"🧪 未來將整合 TensorRT engine：{engine_path}")
